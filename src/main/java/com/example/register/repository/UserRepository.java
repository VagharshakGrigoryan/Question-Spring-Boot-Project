...
public interface UserRepository extends PagingAndSortingRepository<User, Long> {
    ...
    int countCorrectAnswers(String username);

    @Query("SELECT u.username"
            + " FROM User u, IN(u.answers) ans"
            + " WHERE KEY(ans).id = :questionId AND ans <> '' ")
    Set<String> getUsernamesThatAnswered(long questionId);

    @Query("SELECT u.username"
            + " FROM User u, IN(u.answers) ans"
            + " WHERE KEY(ans).id = :questionId AND ans = :answer")
    Set<String> getUsernamesThatAnsweredWith(long questionId, String answer);
    ...
    Set<String> getUsernamesThatAnsweredEveryQuestion();
    ...
    Set<String> getUsernamesThatCorrectlyAnsweredEveryQuestion();
